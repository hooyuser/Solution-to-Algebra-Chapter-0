...
\begin{solution}
	\begin{itemize}
		...
		\item If $A$ is finite, then $F^{ab}(A)$ is finite. $F^{ab}(B)\cong F^{ab}(A)$ guarantees that $F^{ab}(B)$ is finite. Hence we see $B$ is finite. Furthermore it follows that
		\[
		|F^{ab}(A)/\sim| = |F^{ab}(B)/\sim|\implies 2^{|A|}=2^{|B|}\implies |A|=|B|.
		\]
		That is, $A\cong B$ in $\Set$.
	\end{itemize}
\end{solution}
...
\begin{problem}[6.7]
$\neg$ Show that inner automorphisms (cf. \hyperlink{Exercise 4.8}{Exercise 4.8}) form a subgroup of
...
$ for $n\in\N_+$. Since the subgroups of a cyclic group are also cyclic, we see finitely generated subgroup $\langle a_1,a_2,\cdots,a_n\rangle$ is cyclic.	 
\end{solution}	
...
\begin{problem}[7.1]
$\vartriangleright$ List all subgroups of $S_3$ (cf. \hyperlink{Exercise 6.13}{Exercise 6.13}) and determine which subgroups are normal and which are not normal. [\textsection7.1]
\end{problem}
\begin{solution}
...
\begin{problem}[7.4]
Prove that the relation defined in \hyperlink{Exercise 5.10}{Exercise 5.10} on a free abelian group $F =F^{ab}(A)$ is compatible with the group structure. Determine the quotient $F/\sim$ as a better known group.
\end{problem}
\begin{solution}
For all $f,f',h\in F$,
\[
...
\end{problem}
\begin{solution}
	Take $H=C_3$, the cyclic group of order $3$. Take $G_1=D_6$ and $G_2=C_6$, then one sees both $G_1/H$ and  $G_2/H$ are $C_2$. But obviously $G_1$ and $G_2$ are not isomorphic, one being abelian while the other is not.
\end{solution}
...
\end{itemize}
To sum up, $gH=Hg$ follows for all $g\in G$, which implies $H$ is normal in $G$.
\end{solution}
